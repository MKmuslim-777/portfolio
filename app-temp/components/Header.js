                ...
                <nav className="flex flex-col gap-2">
                  {navItems.map((item, index) => (
                    <motion.button
                      ...
                      whileHover={{ x: 5 }}
                      onClick={() => scrollToSection(item.id)}
                      className={`px-4 py-3 rounded-xl text-left text-sm font-medium uppercase tracking-wider transition-all duration-300 cursor-pointer ${
                        activeSection === item.id
                          ? "bg-primary text-white shadow-lg shadow-primary/30"
                          : "text-gray-700 dark:text-gray-200 hover:bg-white/10 dark:hover:bg-black/20 hover:text-primary"
                      }`}
                    >
                      {item.name}
                    </motion.button>
